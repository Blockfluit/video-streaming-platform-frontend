...
export const useEditStore = defineStore("editStore", {
    ...
    actions: {
        async updateMedia(mediaId, thumbnail, type, genres, actors, trailer, year, plot, videosOrder, updateVideos) {
            ...
            if (genres.toString() !== media.value.genres.toString()) formData.append("genres", genres)
            if (actors.toString() !== media.value.actors.toString()) formData.append("actors", actorIds)
